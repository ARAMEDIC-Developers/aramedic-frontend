    ...
    conexion.query(validar, [DNI], async function(error, rows) {
        ...
        if (rows.length < 1) {
            ...
            function manejarInicioSesion(res, req, usuario, datos, dashboard, idKey) {
                ...
                req.session.idusu = usuario.dni; // Cambiado de id a dni
                ...
                req.session[idKey] = datos[idKey]; // ID del médico o paciente
                console.log(req.session);
                res.redirect(dashboard);
            }

            function procesarLoginPorRol(res, req, usuario) {
                conexion.query(loginQuery, [usuario.rol_id, usuario.dni], function (error, result) {
                    if (error) {
                    ...
                    if (usuario.rol_id === 1) {
                        ...
                        // Manejo para otros roles (e.g., admin)
                        const mensaje = "Rol no soportado.";
                        return res.render("login", { mensaje, link, oldData: req.body });
                    }
                });
            }
